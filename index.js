	...
	client.on('message-new', async (mek) => {
		...
		case 'cuttly':
				...
				case 'wallgravity':
					...
					reply(ind.wait())
					yogi = await getBuffer(`http://lolhuman.herokuapp.com/api/textprome2/wallgravity?apikey=RamlanID&text1=${t1}&text2=${t2}`)
					client.sendMessage(from, yogi, image, {quoted: mek})
					await limitAdd(sender)
				break
				case 'space':
					...
					reply(ind.wait())
					yogi = await getBuffer(`http://lolhuman.herokuapp.com/api/textprome2/space?apikey=RamlanID&text1=${t1}&text2=${t2}`)
					client.sendMessage(from, yogi, image, {quoted: mek})
					await limitAdd(sender)
				break
				case 'marvelstudio':
					...
					reply(ind.wait())
					yogi = await getBuffer(`http://lolhuman.herokuapp.com/api/textprome2/marvelstudio?apikey=RamlanID&text1=${t1}&text2=${t2}`)
					client.sendMessage(from, yogi, image, {quoted: mek})
					await limitAdd(sender)
				break
		case 'tinyurl':
...
//dappa gay
	...
	break
		...
		case 'fftourserti': 				
				...
				client.sendMessage(from, buffer, image, {quoted: mek, caption: '*Nih kak.. jangan lupa follow ig @yogi.prwaa._*'})
				break
		case 'fftourserti2': 				
				if (!isRegistered) return reply( ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
